...
BEGIN
    ...
    raise_application_error(-20000,'Una persona sin ningún titulo profesional no puede ser profesor');
    END IF;
END TR_Pro_Titu;
/
--UPDATE
...
BEGIN
    ...
    ELSE raise_application_error(-20000, 'No hay cupos disponibles en este grupo');
    END IF;
END TR_CUPOS_DISP;
/
...
BEGIN
    ...
    raise_application_error(-20000,'No se puede modificar el numero de carné');
    END IF;
END TR_Est_NoCamb;
/
--BORRAR
...
FOR EACH ROW
BEGIN
    raise_application_error(-20000, 'No se puede eliminar una materia');
...
FOR EACH ROW
BEGIN
    raise_application_error(-20000, 'No se puede eliminar una decanatura');
